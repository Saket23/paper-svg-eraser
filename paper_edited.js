 ...
 */
 ...
 var paper = function(self, undefined) {
    ...
    var Curve = Base.extend({
        ...
        splitAt: function(location, location1) {
            console.log('saket splitAt', location.index, location1.index)
            var path = this._path;
            return path ? path.splitAt(location, location1) : null;
        },
    
        splitAtTime: function(time, time1, curve2) {
            console.log('saket splitAtTime', time, time1, curve2)
            return this.splitAt(this.getLocationAtTime(time), this.getLocationAtTimeCurve(time1, curve2));
        },
        ...
        },
    {
        ...
        getLocationAtTime: function(t) {
            console.log('saket this', this)
            return t != null && t >= 0 && t <= 1
                    ? new CurveLocation(this, t)
                    : null;
        },

        getLocationAtTimeCurve: function(t, curve){
            console.log('saket curve', curve)
            return t != null && t >= 0 && t <= 1
            ? new CurveLocation(curve, t)
            : null;
        },
    ...
    var CurveLocation = Base.extend({
        ...
        split: function(curveLocation) {
                var curve = this.getCurve();
                var curve2 = curveLocation.getCurve();
                console.log('saket lksdnvlkew', curve)
                var path = curve._path,
                res = curve && curve.splitAtTime(this.getTime(), curveLocation.getTime(), curve2);
            if (res) {
    ...
    var Path = PathItem.extend({
        ...
        splitAt: function(location, location2) {
            var loc = this.getLocationAt(location),
                index = loc && loc.index,
                time = loc && loc.time,
                tMin = 1e-8,
                ...
                var loc2 = this.getLocationAt(location2),
            ...
            if (index >= 0 && index < curves.length) {
                ...
                if (time2 >= tMin) {
                    curves[index2++].divideAtTime(time2);
                }
                console.log('saket remove', index, index2)
                var segs = this.removeSegments(index, index2, true),
                    path;
                if (this._closed) {
    ...
    PathItem.inject(new function() {
        ...
        function splitBoolean(path1, path2, operation) {
            var _path1 = preparePath(path1),
                ...
                console.log('names', crossings)
                let crossingsForCouple = [];
                let k = 0;
                while(k <= crossings.length - 1){
                    const arr1 = [crossings[k], crossings[k + 2]]
                    crossingsForCouple.push(arr1)
                ...
                console.log('saket yo', crossingsForCouple)
            ...
            for (var i = crossingsForCouple.length - 1; i >= 0; i = i - 1) {
                var path = crossingsForCouple[i][0].split(crossingsForCouple[i][1]);
                if (path) {
                    if (addPath(path)){
                        path.getFirstSegment()?.setHandleIn(0, 0);
                    }
